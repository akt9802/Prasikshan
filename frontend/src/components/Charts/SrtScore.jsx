...
function SrtScore({ userDetails }) {
  //   console.log("SRT userDetails:", userDetails); // Debug log

  const processSRTData = () => {
    try {
      if (
        !userDetails ||
      ...
      const srtData = Object.entries(dailyCounts)
        .map(([date, count]) => ({
          date,
      ...
      //   console.log("Processed SRT data (last 30 days):", srtData); // Debug log
      return srtData;
    } catch (error) {
      console.error("Error processing SRT data:", error);
      return [];
    }
  };

  const srtData = processSRTData();
  const totalTests = srtData.reduce((sum, day) => sum + day.testsCount, 0);
  const activeDays = srtData.length;
  ...
  const dataToUse = srtData.length > 0 ? srtData : defaultData;
  ...
  return (
    ...
    >
      ...
      >
        SRT Test Activity - Last 30 Days{" "}
        {srtData.length > 0 && `(${activeDays} Active Days)`}
      </h2>
      ...
      >
        ...
        <div style={{ textAlign: "center" }}>
          ...
          >
            {srtData.length > 0
              ? Math.max(...srtData.map((d) => d.testsCount))
              : 0}
          </div>
      ...
      <div style={{ textAlign: "center", marginTop: 15 }}>
        ...
        >
          {srtData.length === 0
            ? "🎯 Take your first SRT test in the last 30 days to see activity!"
            : totalTests >= 10
            ? "🎉 Great SRT activity in the last 30 days!"
            : totalTests >= 5
            ? ""
            : ""}
        </span>
